...
export default class MakeInstaller extends InstallerBase {
  ...
  constructor(
    ...
    exeFileFinder: IExecutableFileFinder = new ExecutableFileFinder(MAKE_CLI_NAME, makeProvider),
    cache: ICache = new Cache(version, MAKE_CLI_NAME)) {
    super(makeProvider)
  ...
  private getUrl(os: string, arch: string = 'x86_64', ext: string = 'tar.gz'): string {
    // return `https://github.com/Kitware/CMake/releases/download/v${this._version}/${MAKE_CLI_NAME}-${this._version}-${os}-${arch}.${ext}`
    return ''
  }
  ...
  protected async installInternal(): Promise<void> {
    ...
    case 'linux':
      ...
      dlCommand = `curl -L ${urlLinux} | tar xz --wildcards --strip-components=1 -C ${this.INSTALL_DIR} \'*/${MAKE_CLI_NAME}\'`
      break
    case 'darwin':
      const urlDarwin: string = this.getUrl('Darwin')
      dlCommand = `curl --insecure -L ${urlDarwin} | tar xz --strip-components=1 --include \'*/${MAKE_CLI_NAME}\' -C ${this.INSTALL_DIR}`
      break
    case 'win32':
      const urlWindows: string = this.getUrl('win32', 'x86', 'zip')
      const zipPath: string = path.join(this.INSTALL_DIR, `${MAKE_CLI_NAME}.zip`)
    ...
    this._log.info(`Executing command below to install ${MAKE_CLI_NAME}...`)
